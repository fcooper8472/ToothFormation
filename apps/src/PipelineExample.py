...
executable = os.path.join(chaste_build_dir, 'projects/ToothFormation/apps', 'PipelineExample')
path_to_output = os.path.join(chaste_test_dir, 'TestPipelineExample')
...
def combine_output():
    ...
    # Sanity check on size of the combined results file
...
def generate_html():
    ...
    header_row = [header_row[n].replace('_', ' ').title() for n in range(len(header_row))]
