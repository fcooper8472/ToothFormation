...
command_line_args = {
    ...
    'SM': {'name': 'inner_corner_stiffness_mult',
           'vals': np.linspace(0.02 * ref_sm, 0.05 * ref_sm, num=1)},
    'AAM': {'name': 'ap-ap_mult',
            'vals': np.linspace(3.0 * ref_aam, 4.0 * ref_aam, num=1)},
    'NS': {'name': 'normal_str',
           'vals': [ref_ns]},
    'CYF': {'name': 'cyclic_freq',
            'vals': np.logspace(-3.0, -3.0, num=1, base=2.0)},
    'GOP': {'name': 'grad_on_prop',
            'vals': np.linspace(0.75 * ref_gop, 0.5 * ref_gop, num=1)},
